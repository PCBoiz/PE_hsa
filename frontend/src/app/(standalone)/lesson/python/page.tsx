...
// Port lesson_python.html — markup 1:1. CSS: lesson.css + chatbot.css + inline style
// gốc (lesson_python.inline.css). Inline script → /static/js/pages/lesson_python.inline.js.
// {{ gems }}/{{ streak }} server bơm → lesson-topbar.hydrate.js (fetch /api/user + /api/stats).
...
const SCRIPTS = [
  ...
  '/static/js/pages/lesson_python.tailwind.js',
  'https://cdn.jsdelivr.net/npm/canvas-confetti@1.5.1/dist/confetti.browser.min.js',
  '/static/js/pages/lesson_python.inline.js',
  '/static/js/main.js',
...
export default function LessonPythonPage() {
  return (
    <div className="lesson-page h-screen flex flex-col overflow-hidden">
      <PageStyles hrefs={["/static/css/lesson.css","/static/css/pages/lesson_python.inline.css","/static/css/chatbot.css"]} />
      <title>Python - Phân Loại Gói Hàng</title>
      ...
      <nav className="h-20 border-b-2 border-gray-100 flex items-center justify-between px-10 bg-white shrink-0">
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-2">
            <div className="w-10 h-10 bg-brand-primary rounded-xl flex items-center justify-center text-white text-xl shadow-lg shadow-red-100">
      ...
      <main className="flex-1 flex overflow-hidden">
        <section className="w-1/2 p-12 overflow-y-auto bg-white flex flex-col items-center">
          <div className="w-full max-w-xl">
            <div className="mb-10 text-center">
              <h2 className="text-4xl font-extrabold text-gray-800 mb-4">Phân Loại Gói Hàng! 📦</h2>
              <p className="text-gray-500 text-lg font-medium">Kéo các khối lệnh để giúp robot giao hàng quyết định khi nào giao ngay.</p>
            </div>
            ...
            <div className="mt-12 bg-white rounded-3xl p-6 border-2 border-dashed border-gray-200">
              ...
              <div className="flex flex-wrap justify-center gap-4">
                <div className="logic-card block-red" draggable data-type="cond" data-val="km <= 10">
                  <i className="fa-solid fa-location-dot"></i> km &lt;= 10
                </div>
                <div className="logic-card block-red" draggable data-type="cond" data-val="km > 100">
                  <i className="fa-solid fa-road"></i> km &gt; 100
                </div>
                <div className="logic-card block-blue" draggable data-type="act" data-val="deliver_now()">
                  <i className="fa-solid fa-truck-fast"></i> Giao ngay
                </div>
                <div className="logic-card block-blue" draggable data-type="act" data-val="schedule_later()">
                  <i className="fa-solid fa-calendar"></i> Hẹn lịch
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="w-1/2 p-12 bg-[#F7F9FC] flex flex-col items-center">
          <div className="w-full max-w-xl h-full flex flex-col code-window shadow-2xl shadow-red-900/10">
            <div className="h-12 border-b border-white/5 flex items-center px-6 gap-2">
              <div className="w-3 h-3 rounded-full bg-[#FF5F56]"></div>
              <div className="w-3 h-3 rounded-full bg-[#FFBD2E]"></div>
              <div className="w-3 h-3 rounded-full bg-[#27C93F]"></div>
              <span className="ml-4 text-white/30 font-mono text-xs font-bold uppercase tracking-widest">Python 3.12</span>
            </div>

            <div className="flex-1 p-10 font-mono text-xl leading-relaxed overflow-y-auto">
              <div className="flex items-center gap-3 mb-6">
                <span className="text-brand-primary">distance</span>
                <span className="text-white/40">=</span>
                <span className="text-orange-400">5</span>
                <span className="text-white/20 text-sm italic"># km</span>
              </div>
              <div className="space-y-4">
                ...
                <div className="pl-12 text-green-400">schedule_later()</div>
              </div>
            ...
            <div className="p-8 bg-[#222222] border-t border-white/5 flex justify-between items-center rounded-b-[16px]">
              <p id="hint-box" className="text-xs text-white/30 font-medium max-w-[200px]">Hãy kéo thử khối &apos;km &lt;= 10&apos; vào ô điều kiện nhé!</p>
              <button onClick={() => W().runMission()} className="btn-3d px-10 py-4 bg-brand-primary text-white font-extrabold rounded-2xl flex items-center gap-3 hover:bg-brand-secondary shadow-lg shadow-red-900/20 active:scale-95 transition-all uppercase tracking-tight">
      ...
      <div id="success-modal" className="fixed inset-0 z-50 bg-white/90 backdrop-blur-md hidden flex items-center justify-center">
        <div className="text-center p-12 bg-white rounded-[40px] shadow-2xl border-4 border-brand-primary max-w-sm w-full mx-4">
          <div className="w-24 h-24 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-6 text-5xl">🎉</div>
          <h3 className="text-3xl font-black text-gray-800 mb-2">QUÁ ĐỈNH!</h3>
          <p className="text-gray-500 font-medium mb-8">Bạn đã hiểu câu lệnh điều kiện Python rồi! +50 XP</p>
