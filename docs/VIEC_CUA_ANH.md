...
| **K3** | **Tick hai ô: `G24` và `G19`** trong bảng phân rã (STT 22 — record Zoom; STT 17 — giáo viên giao bài). Tôi không tự tick được: Google chặn đăng nhập trong trình duyệt do công cụ điều khiển, connector Drive chỉ sửa được tiêu đề tệp chứ không ghi ô, và Chrome/Edge bản 153 chặn gỡ lỗi từ xa trên profile mặc định. | Dòng 22 đủ tám gạch, dòng 17 đủ mười hai gạch — đều đo trên màn thật và soi ảnh 26/09 (dòng 17: "Đổi người nhận" đã có sẵn, ô "Sửa bài" thêm hôm nay, bảng chấm ghi rõ từng em "Chưa nộp"). | 20 giây |
...
| **D1** | **Kho tài liệu Cloudflare R2** (anh đã chọn làm) — tạo tài khoản Cloudflare → R2 → tạo bucket → tạo API token, gửi tôi bốn giá trị (Account ID, Access Key, Secret, tên bucket). | Đóng dòng 30 (học liệu) và phần nộp tệp của dòng 31: PDF/slide của lớp và bài học sinh nộp cần chỗ lưu ngoài CSDL. | 10 phút |
...
1. `backend/.env` — dòng `SECRET_KEY=` (để máy anh chạy giống production) — *(Đính chính 15/09: SAI. Cùng một khoá ở hai nơi nghĩa là thẻ cấp trên máy dev dùng được trên production; máy dev phải dùng khoá KHÁC — xem A6.)*
