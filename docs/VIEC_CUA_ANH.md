...
| **C1-cloud** | **Cho phiên Claude cloud quyền ĐẨY code lên repo.** Mở claude.ai/code (đang đăng nhập sonthaiha07@gmail.com) → biểu tượng GitHub / Settings → **Connect GitHub** (hoặc "Install GitHub App") → chọn tài khoản GitHub **PCBoiz** → cho phép repo **PE_hsa** → Save. | Ba phiên cloud tôi mở 25/09 (chạy test, E2 thông báo, E3 hộp Yêu cầu) clone được repo (công khai) nhưng KHÔNG đẩy được kết quả lên (GitHub chỉ có master, erp). Có quyền thì mỗi phiên đẩy một nhánh `cloud/...`, tôi xem và gộp vào `erp`. Tiền tính vào 100 USD "Cloud session credits" của tài khoản sonthaiha07 (hết hạn 05/11). | 3 phút |
