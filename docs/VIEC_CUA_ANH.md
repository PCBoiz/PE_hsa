...
| **N4** | **Đẩy code lên NGAY khi tiện** (`git push origin master`), nên buổi tối lúc không ai học. **Lưu ý (tôi phát hiện 25/09):** trang thật đã có bản tới `bd58824` (đẩy sau lúc tạm dừng 24/09) — tức đã có "môn mở qua lớp" (1.3) và chuyển lớp (1.2c). Bản đó có một lỗi tôi gây ra: **ghi chú lúc chuyển lớp hiện trên báo cáo gửi phụ huynh như "nhận xét của giảng viên"**. Bản sửa nằm trong lượt đẩy tới (`e328ade`). Tới lúc đẩy: **đừng chuyển lớp kèm ghi chú**, và đừng gửi báo cáo phụ huynh cho em vừa chuyển lớp. | Đẩy = cập nhật trang thật (web ~3 phút, máy chủ ~45 phút). Lượt tới gồm: sửa lỗi trên, bỏ Thi thử (anh đã chốt), điền "lần cuối hoạt động" cho mọi tài khoản, gỡ nút Đăng ký cuối cùng, Tổng quan ghi lớp mới khi chuyển lớp. Đã đo đủ trên máy dev. | 1 phút |
