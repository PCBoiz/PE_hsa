...
def chon_the_tom_tat(khoa, so, the_day_du, cards_tom_tat):
    """Thẻ tóm tắt nhận bản sao: ghi tay nếu có, không thì thẻ trùng nhiều từ nhất."""
    if (khoa, so) in THE_TOM_TAT:
        return THE_TOM_TAT[(khoa, so)]
    if not cards_tom_tat:
        return None
    goc = _tu(the_day_du.get('title'))
    diem = [len(goc & (_tu(k.get('title')) | _tu(k.get('body')))) for k in cards_tom_tat]
    return max(range(len(cards_tom_tat)), key=lambda i: diem[i])
...
def ghep(rows, ghi):
    ...
    for khoa, bo in BO_MINH_HOA.items():
        for so, (the, visual) in sorted(bo.items()):
